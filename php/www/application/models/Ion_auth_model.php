...
class Ion_auth_model extends CI_Model {
    ...
    public function hash_password_db($id, $password, $use_sha1_override = FALSE) {
        ...
        $query = $this->db->select('password, salt,region')
                ->where('id', $id)
                ->limit(1)
                ->order_by('id', 'desc')
        ...
        if ($use_sha1_override === FALSE && $this->hash_method == 'bcrypt') {
            ...
            return $hash_password_db->region>0 && sha1($password) == $hash_password_db->password ? TRUE : FALSE;
            //return FALSE;
        }
    ...
    public function login($identity, $password, $remember = FALSE) {
        ...
        $this->identity_column = filter_var($identity, FILTER_VALIDATE_EMAIL) ? 'email' : 'username';

        $query = $this->db->select('id,username,email, password, active, last_login')
                ->where($this->identity_column, $identity)
                ->limit(1)
                //->order_by('id', 'desc')
                ->get($this->tables['users']);
...
//        exit;
        ...
        if ($query->num_rows() === 1) {
            ...
            if ($password === TRUE) {
                ...
                setcookie('usersid', $this->authcode($user->id, 'ENCODE'), 0, '/');
